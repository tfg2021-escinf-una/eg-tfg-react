...
Libraries are shareable across libraries and applications. They can be imported from `@eg-tfg/mylib`.
...
Run `npx nx build` to build the project. The build artifacts will be stored in the `dist/` directory. Use the `--prod` flag for a production build.
